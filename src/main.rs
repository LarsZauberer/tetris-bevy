...
use tetris_bevy::utils::{
    ...
    get_rotation, hex_color, row_clearing, valid_position,
};
...
fn game_loop(
    ...
    if ticker.0.tick(time.delta()).just_finished() {
        ...
        let locations = get_rotation(&locations, &world.current.rotation, &world.current.kind);
    ...
    if keys.just_pressed(KeyCode::KeyA) {
        world.current.rotation = world.current.rotation.rot_left();
        rotation_undo = 1;
    } else if keys.just_pressed(KeyCode::KeyD) {
        world.current.rotation = world.current.rotation.rot_right();
        rotation_undo = 2;
    }
    ...
    if valid_position(&world, &locations, offset) {
        ...
        match rotation_undo {
            1 => world.current.rotation = world.current.rotation.rot_right(),
            2 => world.current.rotation = world.current.rotation.rot_left(),
            _ => (),
        }
    ...
    if keys.just_pressed(KeyCode::Space) {
        ...
        let locations = get_locations(world.current.kind);
        let mut locations = get_rotation(&locations, &world.current.rotation, &world.current.kind);
        ...
        while valid_position(&world, &locations, offset) {
            ...
            locations = get_rotation(&locations, &world.current.rotation, &world.current.kind);
